...
@pytest.mark.django_db
def test_get_group_by_meta_data_in_rows_with_date_field(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(user=user)
    ...
    handler = ViewHandler()
    counts = handler.get_group_by_meta_data_in_rows([date_field], rows, queryset)
    ...
    assert counts == {
        date_field: [
            {
                "count": 2,
                f"field_{date_field.id}": datetime(2010, 1, 1, 12, 0, 0, tzinfo=utc),
            },
            {
                "count": 1,
                f"field_{date_field.id}": datetime(2010, 1, 1, 12, 1, 0, tzinfo=utc),
            },
            {
                "count": 1,
                f"field_{date_field.id}": datetime(2010, 1, 2, 12, 1, 0, tzinfo=utc),
            },
