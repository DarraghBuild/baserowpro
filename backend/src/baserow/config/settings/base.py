...
REDIS_SSL_CA_CERT_PATH = os.getenv("BASEROW_REDIS_SSL_CA_CERT_PATH", "").strip()
if DISABLE_REDIS_SSL_CERT_REQS and REDIS_SSL_CA_CERT_PATH:
    raise ImproperlyConfigured(
        "Can't set both BASEROW_DISABLE_REDIS_SSL_CERT_REQS and "
        "BASEROW_REDIS_SSL_CA_CERT_PATH at the same time "
        "as they are mutually exclusive."
    )

REDIS_IS_USING_TLS = REDIS_PROTOCOL == "rediss" or "rediss" in REDIS_TLS_URL

if not REDIS_IS_USING_TLS and (DISABLE_REDIS_SSL_CERT_REQS or REDIS_SSL_CA_CERT_PATH):
    raise ImproperlyConfigured(
        "Can't see DISABLE_REDIS_SSL_CERT_REQS or "
        "REDIS_SSL_CA_CERT_PATH when the "
        "REDIS_PROTOCOL/REDIS_URL is not using rediss."
    )

if DISABLE_REDIS_SSL_CERT_REQS:
    # We need to set the certificate check to None, otherwise it is not compatible
    # with
    # the `heroku-redis:mini` addon or other Redis servers with self signed certs. The
    # URL generated by that addon is over a secured connection with a self signed
    # certificate. The redis broker could fail if the certificate can't be verified.
...
if REDIS_SSL_CA_CERT_PATH:
    CELERY_REDBEAT_REDIS_USE_SSL = {"ssl_ca_certs": REDIS_SSL_CA_CERT_PATH}

    parsed_redis_url = urlparse(REDIS_TLS_URL)
    ssl_context = ssl.SSLContext()
    ssl_context.load_verify_locations(REDIS_SSL_CA_CERT_PATH)
    CHANNELS_REDIS_HOST = {
        "host": parsed_redis_url.hostname,
        "port": parsed_redis_url.port,
        "username": parsed_redis_url.username,
